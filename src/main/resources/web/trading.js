/* The Trading tab, rendered by the desk. Every number is computed server-side — positions and
   exposure by the ledger, greeks/VaR/PnL by risk-engine's calculators — so this file only formats
   and signs the snapshot that /trading/api/stream pushes. The ledger serves JSON and nothing else;
   this is the only place its state is drawn. */
(function () {
  "use strict";

  var STREAM = "/trading/api/stream";
  ...
  function renderDeadLetters(sync) {
    ...
    // The shell decides what this means for the status bar — the column is also scoped to this
    // tab, and only the shell knows which tab is in front of you.
    window.deskShell.suppress("dlt-item", !(sync.deadLetters > 0));
  }
  ...
  function connect() {
    ...
    stream.onopen = () => {
      ...
      // Reaching the ledger through the gateway is as good a proof that the desk is wired up as
      // an embedded tab finishing its load.
      window.deskShell.markLive();
    };
    stream.onerror = () => {
